...
const getSearchUserByKeyWord = async e => {
    ...
    try {
        const response = await fetch(`http://localhost:3000/client/user/search/${searchWord}`, {
            method: 'POST',
            headers: {
...
const patchInviteUserToChannel = async () => {
    ...
    console.log(userIds);
    try {
        const response = await fetch(`http://localhost:3000/client/channel/invite/${channelId}`, {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
