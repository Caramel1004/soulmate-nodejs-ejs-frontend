...
const postSendChatAndUploadFilesToChatRoom = async () => {
    ...
    try {
        ...
        await fetch('/client/chat/' + channelId + '/' + chatRoomId, {
            method: 'POST',
            body: formData
        });
...
const patchExitChatRoom = async () => {
    try {
        ...
        const response = await fetch(`/client/chat/exit-chat-room/${channelId}/${chatRoomId}`, {
            method: 'PATCH',
            headers: {
        ...
        window.location.href = `/mychannel/${data.channelId}?searchType=chatRooms`;
    } catch (err) {
        console.log(err);
    }
}
...
const getLoadFilesInChatRoom = async activeSubBoardChild => {
    ...
    const response = await fetch(`/client/chat/file-list/${channelId}/${chatRoomId}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json'
        }
