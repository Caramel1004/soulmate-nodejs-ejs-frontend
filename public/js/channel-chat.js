...
const postCreateChatRoom = async channelId => {
    ...
    try {
        const response = await fetch(`/client/chat/${channelId}`, {
            method: 'POST',
            headers: {
...
const getSearchChatRoomsByKeyWord = async e => {
    ...
    let URL = `/mychannel/${channelId}?searchType=chatRooms&searchWord=${searchWord}`;
    if(searchWord == '') {
        URL = `/mychannel/${channelId}?searchType=chatRooms`;
    }
    try {
